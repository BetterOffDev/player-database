...
/*************************************************
	Create the admin areas for rankings updates
*************************************************/

function player_rankings_panel(){
  add_menu_page('Player Rankings', 'Player Rankings', 'edit_posts', 'overall-rankings', 'overall_rankings_func', 'http://draftbreakdown.com/wp-admin/images/generic.png', '7' );
  add_submenu_page( 'overall-rankings', 'QB Rankings', 'QB', 'edit_posts', 'qb-rankings', 'qb_rankings_func');
  add_submenu_page( 'overall-rankings', 'RB Rankings', 'RB', 'edit_posts', 'rb-rankings', 'rb_rankings_func');
  add_submenu_page( 'overall-rankings', 'FB Rankings', 'FB', 'edit_posts', 'fb-rankings', 'fb_rankings_func');
  add_submenu_page( 'overall-rankings', 'WR Rankings', 'WR', 'edit_posts', 'wr-rankings', 'wr_rankings_func');
  add_submenu_page( 'overall-rankings', 'TE Rankings', 'TE', 'edit_posts', 'te-rankings', 'te_rankings_func');
  add_submenu_page( 'overall-rankings', 'OT Rankings', 'OT', 'edit_posts', 'ot-rankings', 'ot_rankings_func');
  add_submenu_page( 'overall-rankings', 'OG Rankings', 'OG', 'edit_posts', 'og-rankings', 'og_rankings_func');
  add_submenu_page( 'overall-rankings', 'C Rankings', 'C', 'edit_posts', 'c-rankings', 'c_rankings_func');
  add_submenu_page( 'overall-rankings', 'DT Rankings', 'DT', 'edit_posts', 'dt-rankings', 'dt_rankings_func');
  add_submenu_page( 'overall-rankings', 'DE Rankings', 'DE', 'edit_posts', 'de-rankings', 'de_rankings_func');
  add_submenu_page( 'overall-rankings', 'ILB Rankings', 'ILB', 'edit_posts', 'ilb-rankings', 'ilb_rankings_func');
  add_submenu_page( 'overall-rankings', 'OLB Rankings', 'OLB', 'edit_posts', 'olb-rankings', 'olb_rankings_func');
  add_submenu_page( 'overall-rankings', 'CB Rankings', 'CB', 'edit_posts', 'cb-rankings', 'cb_rankings_func');
  add_submenu_page( 'overall-rankings', 'S Rankings', 'S', 'edit_posts', 's-rankings', 's_rankings_func');
  // add_submenu_page( 'overall-rankings', 'RS Rankings', 'RS', 'edit_posts', 'rs-rankings', 'rs_rankings_func');
  add_submenu_page( 'overall-rankings', 'LS Rankings', 'LS', 'edit_posts', 'ls-rankings', 'ls_rankings_func');
  add_submenu_page( 'overall-rankings', 'K Rankings', 'K', 'edit_posts', 'k-rankings', 'k_rankings_func');
  add_submenu_page( 'overall-rankings', 'P Rankings', 'P', 'edit_posts', 'p-rankings', 'p_rankings_func');
 // add_submenu_page( 'overall-rankings', '3-4 Defense Rankings', '3-4 Defense', 'manage_options', 'tweener-rankings', 'tweener_rankings_func');

}
//add_action('admin_menu', 'player_rankings_panel');


function overall_rankings_func(){
	
		if ( !empty($_POST['rank'])) {
			
			foreach ( $_POST['rank'] as $player => $rank ) {
			
				update_post_meta( $player, '_overall_rank', $rank );
			...
			<h2>Overall Rankings - '.$class.'</h2>';

			?>
			<div style="float:right;">
				<form method="GET" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
				<strong>Change draft class:&nbsp;&nbsp;</strong><select name="class">
					<option value="2012"<?php if ($class == '2012') echo "selected='selected'"; ?>>2012</option>
					<option value="2013"<?php if ($class == '2013') echo "selected='selected'"; ?>>2013</option>
					<option value="2014"<?php if ($class == '2014') echo "selected='selected'"; ?>>2014</option>
					<option value="2015"<?php if ($class == '2015') echo "selected='selected'"; ?>>2015</option>
				</select>
		...
		<thead>
		    <tr>
		        ...
		        <th>School</th>
				<th>Position</th>
				<?php if ( $class > 2013 ) { echo '<th>New Rank</th>'; } ?>
		    </tr>
		</thead>
		<tfoot>
		    <tr>
		...
		/*	$args = array(
			...
			$args = array(
							'posts_per_page' => -1,
							'post_type' => 'player',
							'meta_key' => '_overall_rank',
							'orderby' => 'meta_value_num',
							'order' => 'ASC'
			);
			
			$overall_rankings = new WP_Query($args);
			$i = 1;
			while ( $overall_rankings->have_posts() ) : $overall_rankings->the_post(); 
			
			
				$id = get_the_ID();
				
				if ( get_post_meta( $id, '_draft_class', true ) != $class ) {
					continue;
				}
			?>
				...
				<tr>
					<td><?php echo $i; ?></td>
					<td><?php the_title(); ?></td>
					<td><?php player_school(); ?></td>
					<td><?php player_position(); ?></td>
					<?php if ( $class > 2013 ) { ?><td><input id="rank" size="5" name="rank[<?php echo $id; ?>]" value="<?php player_overall_rank(); ?>" /></td><?php ; } ?>
				</tr> <?php $i ++; endwhile; ?>
		</tbody>
		</table>
		<p class="submit">
...
function qb_rankings_func(){
		if ( !empty($_POST['rank'])) {
			
			foreach ( $_POST['rank'] as $player => $rank ) {
			
				update_post_meta( $player, '_position_rank', $rank );
		...
		echo '<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
		<h2>Quarterback Rankings - '.$class.'</h2>';
		...
		<tbody>
			...
			$position_rankings = new WP_Query( 'posts_per_page=-1&post_type=player&meta_key=_position_rank&orderby=meta_value_num&order=ASC' );
											
			$i = 1;
			while ( $position_rankings->have_posts() ) : $position_rankings->the_post(); 
				$id = get_the_ID();
				if ( get_post_meta( $id, '_position', true ) != 'QB' ) {
					continue;
				}
				elseif ( get_post_meta( $id, '_draft_class', true ) != $class ) {
					continue;
				}
			?>
				...
				<tr>
					...
					<td><?php player_school(); ?></td>
					<td><?php player_position(); ?></td>
					<?php if ( $class > 2013 ) { ?><td><input id="rank" size="5" name="rank[<?php echo $id; ?>]" value="<?php player_position_rank(); ?>" /></td><?php ; } ?>
				</tr> <?php $i ++; endwhile; ?>
		</tbody>
		</table>
		<p class="submit">
...
function rb_rankings_func(){
		if ( !empty($_POST['rank'])) {
			...
			echo '<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
			<h2>Runningback Rankings - '.$class.'</h2>';
		...
		<tbody>
			...
			while ( $position_rankings->have_posts() ) : $position_rankings->the_post(); 
				$id = get_the_ID();
				if ( get_post_meta( $id, '_position', true ) != 'RB' ) {
					continue;
				}
			...
			?>
				...
				<tr>
					...
					<td><?php player_school(); ?></td>
					<td><?php player_position(); ?></td>
					<?php if ( $class > 2013 ) { ?><td><input id="rank" size="5" name="rank[<?php echo $id; ?>]" value="<?php player_position_rank(); ?>" /></td><?php ; } ?>				</tr> <?php $i ++; endwhile; ?>
		</tbody>
		</table>
		<p class="submit">
		<input type="submit" class="button-primary" value="<?php _e('Save Changes') ?>" />
...
function fb_rankings_func(){
		...
		<tbody>
			...
			while ( $position_rankings->have_posts() ) : $position_rankings->the_post(); 
				$id = get_the_ID();
				if ( get_post_meta( $id, '_position', true ) != 'FB' ) {
					continue;
				}
...
function wr_rankings_func(){
		if ( !empty($_POST['rank'])) {
			...
			echo '<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
			<h2>Wide Receiver Rankings - '.$class.'</h2>';
		...
		<tbody>
			...
			while ( $position_rankings->have_posts() ) : $position_rankings->the_post(); 
				$id = get_the_ID();
				if ( get_post_meta( $id, '_position', true ) != 'WR' ) {
					continue;
				}
...
function te_rankings_func(){
		if ( !empty($_POST['rank'])) {
			...
			echo '<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
			<h2>Tight End Rankings - '.$class.'</h2>';
		...
		<tbody>
			...
			while ( $position_rankings->have_posts() ) : $position_rankings->the_post(); 
				$id = get_the_ID();
				if ( get_post_meta( $id, '_position', true ) != 'TE' ) {
					continue;
				}
...
function ot_rankings_func(){
		if ( !empty($_POST['rank'])) {
			...
			echo '<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
			<h2>Offensive Tackle Rankings - '.$class.'</h2>';
		...
		<tbody>
			...
			while ( $position_rankings->have_posts() ) : $position_rankings->the_post(); 
				$id = get_the_ID();
				if ( get_post_meta( $id, '_position', true ) != 'OT' ) {
					continue;
				}
...
function og_rankings_func(){
		if ( !empty($_POST['rank'])) {
			...
			echo '<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
			<h2>Offensive Guard Rankings - '.$class.'</h2>';
		...
		<tbody>
			...
			while ( $position_rankings->have_posts() ) : $position_rankings->the_post(); 
				$id = get_the_ID();
				if ( get_post_meta( $id, '_position', true ) != 'OG' ) {
					continue;
				}
...
function c_rankings_func(){
		...
		<tbody>
			...
			while ( $position_rankings->have_posts() ) : $position_rankings->the_post(); 
				$id = get_the_ID();
				if ( get_post_meta( $id, '_position', true ) != 'C' ) {
					continue;
				}
...
function dt_rankings_func(){
		if ( !empty($_POST['rank'])) {
			...
			echo '<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
			<h2>Defensive Tackle Rankings - '.$class.'</h2>';
		...
		<tbody>
			...
			while ( $position_rankings->have_posts() ) : $position_rankings->the_post(); 
				$id = get_the_ID();
				if ( get_post_meta( $id, '_position', true ) != 'DT' ) {
					continue;
				}
...
function de_rankings_func(){
		if ( !empty($_POST['rank'])) {
			...
			echo '<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
			<h2>Defensive End Rankings - '.$class.'</h2>';
		...
		<tbody>
			...
			while ( $position_rankings->have_posts() ) : $position_rankings->the_post(); 
				$id = get_the_ID();
				if ( get_post_meta( $id, '_position', true ) != 'DE' ) {
					continue;
				}
...
function ilb_rankings_func(){
		if ( !empty($_POST['rank'])) {
			...
			echo '<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
			<h2>Inside Linebacker Rankings - '.$class.'</h2>';
		...
		<tbody>
			...
			while ( $position_rankings->have_posts() ) : $position_rankings->the_post(); 
				$id = get_the_ID();
				if ( get_post_meta( $id, '_position', true ) != 'ILB' ) {
					continue;
				}
...
function olb_rankings_func(){
		if ( !empty($_POST['rank'])) {
			...
			echo '<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
			<h2>Outside Linebacker Rankings - '.$class.'</h2>';
		...
		<tbody>
			...
			while ( $position_rankings->have_posts() ) : $position_rankings->the_post(); 
				$id = get_the_ID();
				if ( get_post_meta( $id, '_position', true ) != 'OLB' ) {
					continue;
				}
...
function cb_rankings_func(){
		if ( !empty($_POST['rank'])) {
			...
			echo '<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
			<h2>Cornerback Rankings - '.$class.'</h2>';
		...
		<tbody>
			...
			while ( $position_rankings->have_posts() ) : $position_rankings->the_post(); 
				$id = get_the_ID();
				if ( get_post_meta( $id, '_position', true ) != 'CB' ) {
					continue;
				}
...
function s_rankings_func(){
		...
		<tbody>
			...
			while ( $position_rankings->have_posts() ) : $position_rankings->the_post(); 
				$id = get_the_ID();
				if ( get_post_meta( $id, '_position', true ) != 'S' ) {
					continue;
				}
...
function ls_rankings_func(){
		if ( !empty($_POST['rank'])) {
			...
			echo '<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
			<h2>Long Snapper Rankings - '.$class.'</h2>';
		...
		<tbody>
			...
			while ( $position_rankings->have_posts() ) : $position_rankings->the_post(); 
				$id = get_the_ID();
				if ( get_post_meta( $id, '_position', true ) != 'LS' ) {
					continue;
				}
...
function rs_rankings_func(){
		if ( !empty($_POST['rank'])) {
			...
			echo '<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
			<h2>Return Specialist Rankings - '.$class.'</h2>';
		...
		<tbody>
			...
			while ( $position_rankings->have_posts() ) : $position_rankings->the_post(); 
				$id = get_the_ID();
				if ( get_post_meta( $id, '_position', true ) != 'RS' ) {
					continue;
				}
...
function k_rankings_func(){
		...
		<tbody>
			...
			while ( $position_rankings->have_posts() ) : $position_rankings->the_post(); 
				$id = get_the_ID();
				if ( get_post_meta( $id, '_position', true ) != 'K' ) {
					continue;
				}
...
function p_rankings_func(){
		...
		<tbody>
			...
			while ( $position_rankings->have_posts() ) : $position_rankings->the_post(); 
				$id = get_the_ID();
				if ( get_post_meta( $id, '_position', true ) != 'P' ) {
					continue;
				}
...
function tweener_rankings_func(){
		echo '<div class="wrap"><div id="icon-options-general" class="icon32"><br></div>
		<h2>Tweener Rankings</h2></div>';
}

/*************************************************
	Quick Edit Players Admin Page
*************************************************/

function quick_players_edit_panel(){
  add_menu_page('Quick Players Edit', 'Quick Players Edit', 'activate_plugins', 'quick-players-edit', 'quick_players_edit_func', 'http://draftbreakdown.com/wp-admin/images/generic.png', '25' );
  
}

// add_action('admin_menu', 'quick_players_edit_panel');

function quick_players_edit_func(){
		...
		if ( !empty($_POST['height']) ) {
			...
			foreach ( $_POST['height'] as $player => $height ) {
				
				update_post_meta( $player, '_height', $height );
			} 
		}
		
		if ( !empty($_POST['weight'])) {
			
			foreach ($_POST['weight'] as $player => $weight ) {
				
				update_post_meta( $player, '_weight', $weight );
			}
		}
		
		if ( !empty($_POST['dash'])) {
			
			foreach ($_POST['dash'] as $player => $weight ) {
				
				update_post_meta( $player, '_40time', $weight );
			}
		}	if ( isset( $_GET['class'] ) ) {
					$class = $_GET['class'];
				}
				...
				echo '<div class="wrap"><div id="icon-options-general" class="icon32"><br /></div>
				<h2>Quick Rankings Edit - '.$class.'</h2>';
		...
		<thead>
		    <tr>
				...
				<th>Weight</th>
				<th>40 Time</th>
				<?php if ( $class > 2013 ) { echo '<th>Overall Rank</th>'; } ?>
		    </tr>
		</thead>
		<tfoot>
		    <tr>
		...
		<tbody>
			...
			?>
				...
				<tr>
					...
					<td><?php player_school(); ?></td>
					<td><?php player_position(); ?></td>
					<td><input id="height" size="10" name="height[<?php echo $id; ?>]" value="<?php player_height(); ?>" /></td>
					<td><input id="weight" size="10" name="weight[<?php echo $id; ?>]" value="<?php player_weight(); ?>" /></td>
					<td><input id="dash" size="10" name="dash[<?php echo $id; ?>]" value="<?php player_40time(); ?>" /></td>
					<?php if ( $class > 2013 ) { ?><td><input id="rank" size="5" name="rank[<?php echo $id; ?>]" value="<?php player_overall_rank(); ?>" /></td><?php ; } ?>
				</tr> <?php $i ++; endwhile; ?>
		</tbody>
		</table>
